...
echo -e ".\t172800\tIN\tNS\troot-servers." >> /var/lib/knot/root.zone
...
do
  IPV4=${A[${LETTER}]}
  IPV6=${AAAA[${LETTER}]}
  echo -e "root-servers.\t172800\tIN\tA\t${IPV4}" >> /var/lib/knot/root.zone
  if [ ${AAAA[${LETTER}]+isset} ]
  then
    echo -e "root-servers.\t172800\tIN\tAAAA\t${IPV6}" >> /var/lib/knot/root.zone
  fi
done
